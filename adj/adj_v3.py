...
OUTPUT_FOLDER = os.path.join(SCRIPT_DIR, f"{DATASET_NAME}_png_random")
...
if len(user_nodes) < N_USERS_DA_SELEZIONARE:
    selected_users = user_nodes
else:
    selected_users = random.sample(user_nodes, N_USERS_DA_SELEZIONARE)
if len(item_nodes) < N_ITEMS_DA_SELEZIONARE:
    selected_items = item_nodes
else:
    selected_items = random.sample(item_nodes, N_ITEMS_DA_SELEZIONARE)
selected_nodes = selected_users + selected_items
